...
setup('switch livestream to member_only', async ({}) => {
  console.log('\n🔒 Switching livestream to MEMBER_ONLY...\n');
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log('✅ MEMBER_ONLY livestream created');
      console.log('\n✨ Setup completed!\n');
      return;
    }
    ...
    if (livestream.visibility !== 'member_only') {
      ...
      console.log('✅ Visibility set to MEMBER_ONLY');
    } else {
      console.log('✅ Livestream is already MEMBER_ONLY');
    }
